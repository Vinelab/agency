...
<div class="form-group">
    <label class="control-label"> {{Lang::get('album/album.add_photo')}} </label>
</div>
<div class="form-group">
    <div class="col-sm-12">
        <button id="uploader" class="btn btn-primary">{{Lang::get('album/album.add_new_photo')}}</button>
        <button id="choose_from_existing_photos" type="button" class="btn btn-primary" data-toggle="modal" data-target=".selectExistingPhotosModal">{{Lang::get('album/album.choose_from_existing_photos')}}</button>
    </div>
...
<div class="form-group">
    <div id="photos_holder_display">
        ...
        @if($updating && ! $model->photos->isEmpty() || Input::old('photos'))
            @foreach((Input::old('photos') ? Input::old('photos') : $model->photos) as $photo)
