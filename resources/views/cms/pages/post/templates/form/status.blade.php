	...
	<div class="control-group pull-right" id="status-container">
	    <div class="publish-date-margin">
	        @if($updating)
	            ...
	            {{ Form::radio('publish_state[]','editing', ( $edit_post->publish_state == 'editing' ) ? 'true' : '', ['id'=>'editing','class'=>'publish_state']) }}
	            @if (Auth::hasPermission('publish'))
	                {{ Form::label('published',Lang::get('posts/form.publish'), ['class'=>'increase-margin-left']) }}
	                {{ Form::radio('publish_state[]','published',( $edit_post->publish_state == 'published' ) ? 'true' : '' ,['id'=>'published','class'=>'publish_state']) }}

	                {{ Form::label('scheduled',Lang::get('posts/form.schedule'), ['class'=>'increase-margin-left']) }}
	                {{ Form::radio('publish_state[]','scheduled',( $edit_post->publish_state == 'scheduled' ) ? 'true' : '', ['id'=>'scheduled','class'=>'publish_state']) }}
	            @endif
	        @else
	            {{ Form::label('editing',Lang::get('posts/form.editing')) }}
	            {{ Form::radio('publish_state[]','editing','true', ['id'=>'editing','class'=>'publish_state']) }}
	            @if (Auth::hasPermission('publish'))
	                {{ Form::label('published',Lang::get('posts/form.publish'), ['class'=>'increase-margin-left']) }}
	                {{ Form::radio('publish_state[]','published','', ['id'=>'published','class'=>'publish_state']) }}

	                {{ Form::label('scheduled',Lang::get('posts/form.schedule'), ['class'=>'increase-margin-left']) }}
	                {{ Form::radio('publish_state[]','scheduled','', ['id'=>'scheduled','class'=>'publish_state']) }}
	            @endif
	        @endif
	    </div>
